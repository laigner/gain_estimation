...
class GainEstimation:
    ...
    def calculate_gain_per_mode(
        self, pump_power_per_mode: float, length: float, index: int
    ) -> float:
        ...
        if length == self.length_min:
            gain_per_mode = (
                np.sqrt(pump_power_per_mode) * self.gain_inside * self.length_min
            )
        else:
            gain_per_mode = (
    ...
    def calculate_power_after_crystal(self, losses_band, length, index) -> float:
        ...
        if length == self.length_min:
            ...
            ) * 10 ** (-(length - self.length_min) * self.losses_sled / 100)
    ...
    def get_power_after_crystal_list(self) -> list:
        ...
        for index, length in enumerate(self.length_array):
            ...
            losses_band = self.calculate_losses_in_amp_band(
                gain_per_mode=gain_per_mode, length=length, old=old_gain
            )
            # calculate power after crystal
            power_after_crystal = self.calculate_power_after_crystal(
                losses_band=losses_band, length=length, index=index
            )
            # define initial loss
            if length == self.length_min:
                loss = 0.01
    ...
    def plot_amp_loss(self, modes: list = [1], powers: list = [100]):
        ...
        for mode in modes:
            ...
            for power in powers:
                ...
                plt.plot(
                    self.length_array,
                    self.get_power_after_crystal_list()[0],
                    label=f"{power} mW, #Modes: {mode}",
                )
        plt.legend()
        plt.ylim(0, powers[-1])
        plt.xlim(self.length_min, self.length_max)
        plt.xlabel("Length [mm]")
        plt.ylabel("Power [mW]")
        # save figure
    ...
    def plot_loss_pump(self, modes: list = [1], powers: list = [100]):
        fig = plt.figure()
        for mode in modes:
            self.number_of_modes = mode
            for power in powers:
                self.initial_pump_power = power
                plt.plot(self.length_array, self.get_power_after_crystal_list()[1])
        plt.legend()
        plt.ylim(0, powers[-1])
        plt.xlim(self.length_min, self.length_max)
        plt.xlabel("Length [mm]")
        plt.ylabel("Loss [mW]")
        fig.savefig("pump_loss.pdf")
        plt.show()
